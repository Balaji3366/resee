...
export default async function ViewReportPage({ params }: Props) {
  ...
  const { data } = await supabase.from("resume_history").select("*").eq("id", id).single();

  if (!data) {
    return (
      <section className="min-h-screen flex items-center justify-center bg-panel-2">
        <h1 className="font-display text-3xl font-bold text-red-400">Report not found</h1>
      </section>
    );
  }
  ...
  // `report` is written as `JSON.stringify(analysis)` by app/resume/page.tsx
  // (an object with strengths/weaknesses/suggestions arrays) — parse that
  // directly. Fall back to the older "STRENGTHS:/WEAKNESSES:/SUGGESTIONS:"
  // delimited-text regex only for any pre-existing row that isn't valid JSON.
    ...
    <section className="min-h-screen bg-panel-2 py-20">
      <div className="mx-auto max-w-5xl px-6">
        <div className="rounded-3xl bg-panel p-10 shadow-xl">
          ...
          <p className="mt-4 text-center text-slate">{data.file_name}</p>

          {/* ATS Score */}
          <div className="mt-10 rounded-2xl bg-panel-2 p-8">
            <h2 className="font-display text-2xl font-bold text-blue-400">📊 ATS Score</h2>
          ...
          <div className="mt-8 rounded-2xl bg-panel-2 p-8">
            <h2 className="font-display mb-5 text-2xl font-bold text-green-400">💪 Strengths</h2>
          ...
          <div className="mt-8 rounded-2xl bg-panel-2 p-8">
            <h2 className="font-display mb-5 text-2xl font-bold text-yellow-400">⚠ Weaknesses</h2>
          ...
          <div className="mt-8 rounded-2xl bg-panel-2 p-8">
            <h2 className="font-display mb-5 text-2xl font-bold text-purple-400">🚀 Suggestions</h2>
