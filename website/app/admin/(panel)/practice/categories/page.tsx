...
export default function AdminPracticeCategoriesPage() {
  const { data: categories, loading, refetch } = useAdminPracticeCategories();

  const [name, setName] = useState("");
  const [icon, setIcon] = useState("");
  const [submitting, setSubmitting] = useState(false);
  const [editing, setEditing] = useState<AdminPracticeCategory | null>(null);
  const [deleting, setDeleting] = useState<AdminPracticeCategory | null>(null);
  ...
  async function handleSaveEdit() {
    ...
    try {
      const res = await fetch(`/api/admin/practice/categories/${editing.id}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
  ...
  async function handleDelete() {
    ...
    try {
      const res = await fetch(`/api/admin/practice/categories/${deleting.id}`, {
        method: "DELETE",
      });
      const json = await res.json();
  ...
  return (
    <div className="mx-auto max-w-4xl">
      <div className="mb-8">
        ...
        <p className="mt-2 text-slate">Manage the categories used by Practice Topics.</p>
      </div>
                ...
                <div className="flex items-center justify-between gap-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-slate">{category.slug}</p>
                    </div>
                  </div>
