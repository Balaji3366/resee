...
export default function MockTestDetailPage({ params }: { params: Promise<{ testSlug: string }> }) {
  ...
  return (
    <div className="mx-auto max-w-3xl">
      {!sessionActive && <BackNavigation href="/practice/mock-tests" label="Back to Mock Tests" />}
      ...
      {!sessionActive && !loading && test && (
        <div className="rounded-3xl border border-amber/20 bg-panel p-8 shadow-md">
          <div className="flex items-start justify-between gap-6">
            <div>
              <h1 className="font-display text-3xl font-extrabold text-bone">{test.title}</h1>
          ...
          {test.isAvailable && test.bestScore !== null && (
            <p className="mt-6 text-sm font-semibold text-amber">Best score: {test.bestScore}%</p>
          )}
