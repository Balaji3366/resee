...
import type { ContinuePracticeItem, PracticeCategory } from "@/types/practice";

function hrefFor(item: ContinuePracticeItem) {
  return item.kind === "topic" ? `/practice/${item.slug}` : `/practice/mock-tests/${item.slug}`;
}

export default function ContinuePracticePanel({
  items,
  loading,
  categories,
}: {
  items: ContinuePracticeItem[] | null;
  loading: boolean;
  ...
  if (!items || items.length === 0) {
    return (
      <div className="mb-10">
        <PracticeEmptyState categories={categories} />
      </div>
    );
  }
  ...
  return (
    <div className="mb-10 rounded-3xl border border-amber/20 bg-panel p-7 shadow-md">
      <h2 className="text-lg font-semibold text-slate">Continue Practice</h2>

      <div className="mt-4 space-y-4">
        {items.map((item) => (
          <div
            key={item.attemptId}
            className="flex flex-wrap items-center justify-between gap-4 rounded-2xl bg-panel-2/40 p-4"
          >
