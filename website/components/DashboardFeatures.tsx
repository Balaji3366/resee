    ...
    <section className="mt-8">
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-3">
        {TOOLS.map((tool) => (
          <div
            key={tool.title}
            className={`flex items-center gap-3 rounded-xl border border-bone/10 bg-panel px-4 py-3.5 ${
          ...
          >
            ...
            {tool.enabled && tool.href ? (
              <button
                ...
                className="shrink-0 rounded-full p-1.5 text-bone/50 transition hover:bg-bone/5 hover:text-amber"
              >
                <ArrowRight size={15} />
              </button>
            ) : (
